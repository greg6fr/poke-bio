...
require_once __DIR__.\DIRECTORY_SEPARATOR.'SensioFrameworkExtra'.\DIRECTORY_SEPARATOR.'PsrMessageConfig.php';
...
class SensioFrameworkExtraConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
    ...
    public function psrMessage(array $value = []): \Symfony\Config\SensioFrameworkExtra\PsrMessageConfig
    {
        if (null === $this->psrMessage) {
            $this->_usedProperties['psrMessage'] = true;
            $this->psrMessage = new \Symfony\Config\SensioFrameworkExtra\PsrMessageConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "psrMessage()" has already been initialized. You cannot pass values the second time you call psrMessage().');
        }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('psr_message', $value)) {
            $this->_usedProperties['psrMessage'] = true;
            $this->psrMessage = new \Symfony\Config\SensioFrameworkExtra\PsrMessageConfig($value['psr_message']);
            unset($value['psr_message']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['psrMessage'])) {
            $output['psr_message'] = $this->psrMessage->toArray();
        }
